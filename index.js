...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        infoDiv.classList.add("game-card");
        ...
        const game = games[i];
        const information = `<img src=${game.img} class="game-img" /> <p class="game-name"> ${game.name} </p> <p> ${game.description} </p> <p> Backers: ${game.backers} </p>`;
        infoDiv.innerHTML = information;

        // append the game to the games-container
        document.getElementById("games-container").append(infoDiv);
    } 
...
function filterUnfundedOnly() {
    ...
    let goal_unmet = GAMES_JSON.filter( (game) => {
